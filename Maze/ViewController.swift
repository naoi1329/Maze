...
class ViewController: UIViewController {
    ...
    func startAccelerometer() {
        //ハンドラ作成: accelerometerUpdateIntervalで設定した秒数ごとに呼び出されてこれが処理される
        let handler: CMAccelerometerHandler = { (CMAccelerometerData: CMAccelerometerData?, error: Error?) -> Void in
            ...
            if posX >= self.playerView.frame.width  - (self.playerView.frame.width / 2) {
                self.speedX = 0
                posX = self.playerView.frame.width  - (self.playerView.frame.width / 2)
            }
            
            if posY >= self.playerView.frame.height - (self.playerView.frame.height / 2) {
                self.speedY = 0
                posX = self.playerView.frame.height - (self.playerView.frame.height / 2)
            }
            
            for wallRect in self.wallRectArray {
                if(wallRect.intersects(self.playerView.frame)) { //intersects 触れているかのチェック　便利ー
                    self.gameCheck(result: "Game Over!", message: "😇")
                    return
                }
    ...
    func gameCheck(result: String, message: String) {
        ...
        let retryAction = UIAlertAction(title: "もう一度", style: .default) { (action: UIAlertAction) in
            self.retry()
        }
